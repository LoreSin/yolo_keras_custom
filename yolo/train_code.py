...
def _main():
    ...
    # anchor 파일에 따라서 사용되는 모델이 달라짐.
    if is_tiny_version:
        # tiny 모델 사용하는 버젼
        model = create_tiny_model(input_shape, anchors, num_classes,
                                  freeze_body=2, weights_path='model_data/tiny_yolo_weights.h5')
    else:
        # 욜로 모델 사용버젼 = load_pretrained 는 기존 학습된 모델을 취급할때.
    ...
    if True:
        ...
        batch_size = 16  # CPU 에서는 매우 낮춰야 함. = 매우 느림.
        ...
        # 에폭 횟수는, 새로운 트레이닝시 200~ 이상의 많은 횟수 요구
        # 기존트레이닝모델 사용시 적은 횟수로도 빠르게 학습됨.
        model.save_weights(log_dir + 'trained_weights_stage_1.h5')

    # Fine Tuning 을 위한 layer freeze 를 해제
    # 튜닝이 어느정도 이뤄질때 까지 학습.
    if False:
